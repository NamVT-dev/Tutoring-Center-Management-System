...
const autoProcessRequestsJob = () => {
  cron.schedule("*/10 * * * *", async () => {
    ...
    try {
      ...
      const rejectDeadline = moment(now).add(HOURS_AUTO_REJECT, 'hours').toDate();
      const approveDeadline = moment(now).add(HOURS_AUTO_APPROVE, 'hours').toDate();
      ...
      if (expiredTeacherRequests.length > 0) {
        ...
        await ScheduleChangeRequest.updateMany(
          ...
          { 
            $set: { 
              status: "rejected",
              adminResponse: "Hệ thống tự động hủy do Giáo viên được mời không phản hồi kịp thời gian."
            } 
          },
          { session }
        );
      ...
      }).populate("session").session(session);
      ...
      for (const req of requestsToAutoApprove) {
          if (req.session && req.session.startAt < approveDeadline) {
              ...
              req.status = "approved";
              req.adminResponse = "Hệ thống TỰ ĐỘNG DUYỆT do sát giờ học (Admin timeout).";
      ...
      if (approvedRequests.length > 0) {
          console.log(`>> Auto-approved ${approvedRequests.length} requests (Admin timeout).`);
      }
