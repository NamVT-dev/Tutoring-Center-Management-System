...
const updateMyAvailabilityDays = catchAsync(async(req,res,next) => {
    ...
    if(!Array.isArray(payload) || payload.length ===0 ){
        return next(new AppError("days phải là mảng ngày trong tuần (0 = CN... 6 =th7)",400));
    }

    const normalized = [];
    for(const item of payload){
        if(typeof item === "number"){
            if(!isValidDayIndex(item)) throw new AppError(`dayOfWeek không hợp lệ: ${d}`,400);
            normalized.push({ dayOfWeek: item, allowed: true });
        }else if (item && typeof item === "object") {
            const d = item.dayOfWeek;
            if(!isValidDayIndex(d)) throw new AppError(`dayOfWeek không hợp lệ: ${d}`, 400);
        ...
        normalized.push({ dayOfWeek: d, allowed: item.allowed !== false, effective: (effStart||effEnd) ? { start: effStart||undefined, end: effEnd||undefined } : undefined });
        }else{
            throw new AppError("Mỗi phần tử trong days phải là số (0..6) hoặc object {dayOfWeek,...}", 400);
        }
    }
    const map =new Map();
    ...
    if (allowedCount < 3) {
        return next(new AppError("Phải đăng ký ít nhất 3 ngày có thể dạy/tuần", 400));
    }
    const teacher = await User.findOneAndUpdate(
        { _id: teacherId, role: "teacher" },
        { $set: { availability: compact } },
        { new: true, runValidators: true, context: "query" }
    ).select("username email role availability").lean();
