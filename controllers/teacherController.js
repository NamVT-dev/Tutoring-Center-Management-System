...
const registerShiftAvailability = catchAsync(async (req, res, next) => {
  ...
  const definedShiftNames = new Set((cfg.shifts || []).map(s => String(s.name).toUpperCase())); // ví dụ: S1..S6
  const allowedByDay = new Map(
    (cfg.dayShifts || []).map(r => [r.dayOfWeek, new Set((r.shifts || []).map(s => String(s).toUpperCase()))])
  );
  ...
  for (const s of slots) {
    ...
    if (!Array.isArray(s.shifts) || !s.shifts.length === 0)
      return next(
        new AppError("shifts phải là mảng theo ca", 400)
      );
    ...
    if (uniq.length === 0) {
      return next(new AppError("Không có ca hợp lệ. Vui lòng dùng đúng tên ca trong cấu hình trung tâm", 400));
    }
    ...
    if (notAllowed.length) {
      return next(new AppError(`Các ca không được mở cho ngày ${day}: ${notAllowed.join(", ")}`, 400));
    }
  ...
  for (const r of normalized) {
    const prev = byDay.get(r.dayOfWeek) || { dayOfWeek: r.dayOfWeek, shifts: new Set(), effective: r.effective };
  ...
  const compact = Array.from(byDay.values())
    .map(v => ({ dayOfWeek: v.dayOfWeek, shifts: Array.from(v.shifts), effective: v.effective }))
