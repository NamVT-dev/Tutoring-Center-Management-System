...
const updateRoom = catchAsync(async (req,res ,next) =>{
    ...
    if (payload.capacity !== undefined && (!Number.isFinite(payload.capacity) || payload.capacity < 1)) {
...
const deleteRoom = catchAsync(async (req, res, next) => {
  ...
  const inUse = await Session.findOne({
    ...
    status: { $in: ["scheduled"] },
    endAt: { $gte: now },
  }).select("_id startAt endAt status").lean();
  ...
  if (inUse) {
    if (force === "close") {
      ...
      return res.status(200).json({
        status: "success",
        message: "Phòng đang được sử dụng. Đã chuyển trạng thái sang 'closed' thay vì xoá.",
        data: { room: updated },
      });
    }
    return next(new AppError("Phòng đang được sử dụng trong lịch học. Không thể xoá (dùng ?force=close).", 409));
  }
