...
const updateConfig = catchAsync(async(req,res) =>{
    ...
    if (activeDaysOfWeek && !activeDaysOfWeek.every(d => Number.isInteger(d) && d >= 0 && d <= 6)) {
        return next(new AppError("activeDaysOfWeek phải là mảng số trong [0..6]", 400));
    }
    if(shifts) {
        for(const k of ["morning", "afternoon", "evening"]){
            if(!shifts[k]) continue;
            const { startMinute, endMinute } = shifts[k];
            if (!isMinute(startMinute) || !isMinute(endMinute) || endMinute <= startMinute) {
                return next(new AppError(`Shift ${k} không hợp lệ (startMinute/endMinute)`, 400));
            }
        }
    }
    ...
    if(activeDaysOfWeek !== undefined) payload.activeDaysOfWeek = activeDaysOfWeek;
    if(shifts !== undefined){
        payload.shifts = {};
        for (const k of ["morning", "afternoon","evening"]) {
